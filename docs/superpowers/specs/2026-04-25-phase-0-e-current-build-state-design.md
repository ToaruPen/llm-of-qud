...
- `hunger`: string. **Normalized** (markup-stripped, trailing `!` stripped, lowercased) form of CoQ's display bucket. Source: `Stomach.FoodStatus()` returns markup-wrapped strings — `{{g|Sated}}`, `{{W|Hungry}}`, `{{R|Wilted!}}` (only when player has `PhotosyntheticSkin` mutation), `{{R|Famished!}}` (`decompiled/XRL.World.Parts/Stomach.cs:87-102`). We normalize to `"sated" | "hungry" | "wilted" | "famished"`. The bucket-transition logic lives in `UpdateHunger()` (`Stomach.cs:608-643`). Note: `check_status` spec sample at `:462` shows `"satiated"`, but the actual CoQ display string is `"Sated"`; we use the lowercased CoQ form (`"sated"`) and the implementation plan must update `check_status` to match. Integer was dropped because it adds noise the LLM does not use.
- `thirst`: string. **Normalized** (markup-stripped, trailing `!` stripped, lowercased) form of CoQ's `Stomach.WaterStatus()` (`decompiled/XRL.World.Parts/Stomach.cs:104-143`). For non-amphibious bodies, the buckets are `"dehydrated"` (Water `<= 0`), `"parched"`, `"thirsty"`, `"quenched"`, `"tumescent"` (`Water > WATER_QUENCHED`, source `:142`). Thresholds come from `RuleSettings` (`decompiled/XRL.Rules/RuleSettings.cs:13-23`). Amphibious bodies use a different bucket family (`"desiccated" | "dry" | "moist" | "wet" | "soaked"` per `Stomach.cs:104-124`); see "Open hazards" for the v1 policy. The implementation reads `Stomach.WaterStatus()`, strips markup tokens (e.g., `{{R|...}}`), strips a trailing `!` if present, lowercases.
...
5. **Latest-line JSON validity.** Latest `[build]` line on each run passes `json.loads`; `schema == "current_build.v1"`; required keys present (`turn, schema, genotype_kind, genotype_id, subtype_id, level, attributes, hunger, thirst`).
...
7. **Shape parity.** First-turn vs last-turn `[build]` line on the primary run have identical top-level keys.
8. **Phase 0-E specific semantic invariants.** Across non-sentinel turns of both runs:
   ...
   - `hunger` is one of `{sated, hungry, wilted, famished}`. The bucket strings are derived from `Stomach.FoodStatus()` markup-stripped + trailing-`!`-stripped + lowercased (`decompiled/XRL.World.Parts/Stomach.cs:87-102`). `wilted` only occurs when the player has the `PhotosyntheticSkin` mutation; an acceptance run on a non-PhotosyntheticSkin Mutant will not exercise it. A new bucket observed empirically is a hard failure (would require schema documentation update).
   - `thirst` is one of `{tumescent, quenched, thirsty, parched, dehydrated}` for non-amphibious bodies. The full bucket set is derived from `Stomach.WaterStatus()` (`decompiled/XRL.World.Parts/Stomach.cs:104-143`) + `RuleSettings` thresholds (`decompiled/XRL.Rules/RuleSettings.cs:13-23`), markup-stripped + lowercased; a new bucket observed empirically is a hard failure (amphibious-body bucket family is documented in open hazards but is out of scope for the gate).
...
1. **Separate prerequisite docs-only PR** (Phase 0-C precedent — ADR 0003 + ADR 0004 landed before the implementation PR per `docs/adr/0003-phase-0-c-internal-api-pivot.md` + `0004-phase-0-c-csharp-test-infra-deferral.md`): the ADR PR lands first; the implementation PR opens against `main` after the ADR is on `main`. Pro: clean review history (the design pivot is reviewed independently of the C# diff). Pro: if the ADR is rejected / requires major revision, no code thrown away. Con: two PRs, two CI runs, one more merge step.
