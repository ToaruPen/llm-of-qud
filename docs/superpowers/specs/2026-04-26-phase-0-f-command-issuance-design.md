...
- **Hostile interrupt during fallback `PassTurn`.** `decompiled/XRL.Core/ActionManager.cs:834-837` runs `AutoAct.CheckHostileInterrupt()` when the actor is the player and AutoAct is interruptible. **Updated by ADR 0007**: on the success path with `PreventAction = false`, `CommandTakeActionEvent.Check` returns true at `:829-832`, control reaches `:834-837`, and `CheckHostileInterrupt` does fire. AutoAct is typically inactive on Phase 0-F autonomous turns (we do not engage `AutoAct.Setting`), so `IsInterruptable()` returns false and the interrupt block is a no-op in practice; the `[cmd]` line is unaffected. On the abnormal-energy catch path with `PreventAction = true`, the iteration short-circuits at `:829-832` and the interrupt does not fire — same as before. If a future variant calls `PassTurn` from outside `HandleEvent(CommandTakeActionEvent)` — e.g., from `BeginTakeActionEvent` for a "rest" command — the interrupt path becomes relevant.
